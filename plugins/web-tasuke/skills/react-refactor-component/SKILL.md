...
### ルール3: コンポーネント内に`useEffect`を書かない
...
- 同じblock内で複数のドメイン概念(ユーザー情報・支払い情報・通知設定など)を扱っている
...
ファイル名から責務がわからない場合は、import元(使用箇所)もあわせて確認します。

### 2. 違反項目を洗い出す

以下のチェックリストでコードを点検し、違反箇所を列挙します。

- [ ] ルール1: import → デザイン定義 → props定義 → コンポーネントの順序になっているか
- [ ] ルール2: ファイル内のコンポーネントは1つだけか
- [ ] ルール2(補足): ファイル名が`index.tsx`/`index.ts`ではないか(新規/大幅書き換え対象の場合)
- [ ] ルール3: コンポーネント本体に`useEffect`が直接書かれていないか
- [ ] ルール4: 関数の冒頭から最初のJSX(`return (`直前)までが肥大化していないか
- [ ] ルール4: JSX内に「読みづらさ」が同居していないか(重複する構造、深い条件分岐、外側の準備変数の多さなど)
