...
TypeScriptの型の活用、命名規則、エラーハンドリングなどの知識をスキルとして提供します。
Claudeが関連するコンテキストを検出すると自動的に参照するため、
ユーザーが明示的に呼び出す必要はありません。

### MCPサーバー

このプラグインは[Context7](https://context7.com/)のMCPサーバーを同梱しています。
プラグインを有効化するだけで、ライブラリやフレームワークの最新ドキュメントを参照するツールが利用可能になります。
