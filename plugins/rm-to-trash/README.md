...
`rm <ファイル>`を`trash`へ自動的に書き換えてから実行します。

trash-cliはrm互換のために`-r`/`-R`/`-f`/`-d`/`-i`/`-v`と、
その長形式(`--recursive`/`--force`/`--directory`/`--interactive`/`--verbose`)を受理するため、
これらの互換フラグだけが付いた`rm`も書き換えます。

trash-cliが受理しないフラグ(`-I`, `--one-file-system`, `--no-preserve-root`など)が含まれる場合は、
書き換えると挙動が変わってしまうため書き換えずに通常の承認フロー(deny含む)に任せます。
