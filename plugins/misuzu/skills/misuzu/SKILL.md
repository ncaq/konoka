...
`summaryComment`はPR全体への総括コメントです。
スレッド返信を俯瞰する総括であり、
スレッド返信に書いた内容をもう一度並べる場所ではありません。

スレッド返信も総括コメントも同じPRのページに並んで表示されるため、
...
- レビュー本文(スレッドではない`reviews[].body`)やPR全体のコメントへの応答。
  これらには返信するスレッドがないので、総括コメントでしか応答できません。
- 全体検証の結果や、レビュアーに次に見てほしいところ。

個別のスレッドに触れる必要がある時も、
修正内容を説明し直さずに一行の言及やリンクに留めてください。

書くべき情報がない場合は`summaryComment`を省略してください。
無理に総括を作る必要はありません。
