---
name: research
description: Gather up-to-date information from external sources (official docs, GitHub, MDN, Microsoft Learn, NixOS, web search) when local code and prior knowledge are not enough. Use proactively when comparing libraries or frameworks, looking up unfamiliar APIs/specs/syntax, evaluating which approach or library to adopt, investigating library bugs or unexpected behavior, checking current best practices, or surveying ecosystem trends. Triggers include "X vs Y", "how does X work", "what is the recommended way to X", "is X still maintained", "does X support Y", or any task needing fresh information beyond the local codebase. Runs investigations in an isolated context so the main conversation stays uncluttered.
argument-hint: "query"
context: fork
