...
専門レビュースキルをサブエージェントとして並列起動して包括的なコードレビューを行うClaude Codeプラグインです。
...
取得できなかった項目は`unknown`として表示され、
リンクも貼られません。

kyoseiバージョンとモデルにはリンクを貼りません。
kyoseiのプラグインバージョンに対応するGitタグはkonokaに存在せず(konokaのタグはマーケットプレイスバージョン)、
...
`/kyosei`本体から起動される内部用のレビュースキルです。
フロントマターの`context: fork`と`background: true`により、
独立したサブエージェントとしてバックグラウンドで並列起動されることを、
宣言的に保証しています。

`background`フィールドはClaude Code v2.1.218以降で有効です。
それより前のバージョンや非対話モード(`-p`フラグ、Agent SDK)では、
各スキルの結果をその場で待つ動作になりますが、
レビュー自体は同様に完了します。
